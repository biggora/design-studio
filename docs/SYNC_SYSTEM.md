...
- `props` = `{ mockup_tshirt: <url> }`, where `<url>` is built from `externalImageUrl` by `buildMockupTshirtUrl` (same `https://<host>/image.<A>.<B>/` prefix, with Redbubble's signed adult classic-tee flatlay suffix appended); `props` is `null` only if the image URL doesn't match the expected Redbubble shape. For a row that already exists in the database, `mockup_tshirt` is backfilled the same way from the row's own `externalImageUrl` whenever it's missing from the row's existing `props` — any other keys already present in `props` are preserved, and an existing `mockup_tshirt` is never overwritten. This backfill runs independently of `collectionsComplete` (§6.2): even when the collections crawl is incomplete and `collection` is left untouched, a missing `mockup_tshirt` is still added via a `props`-only update.
...
2. Reads which of the batch's `externalId`s already exist in Supabase, in chunks of 100 (`.select("externalId, props, externalImageUrl").in("externalId", ids)` — `props`/`externalImageUrl` are read only to backfill `props.mockup_tshirt`, §3.5; no other curated column is ever read back), and separately checks `title` collisions against the DB in chunks of 25, via `.filter("title", "in", "(\"a\",\"b\")")` with each value double-quoted and internal `"`/`\` escaped (`.in()` quotes but does not escape embedded quotes, which breaks on titles like `24", 36" Print`).
